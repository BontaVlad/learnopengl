...
proc set*(shader: Shader, name: string, value: var Vec3f) =
  glUniform3fv(glGetUniformLocation(shader.id, name.cstring), 1, addr value[0].GLfloat)

proc set*(shader: Shader, name: string, value: var Mat4[float32]) =
  glUniformMatrix4fv(glGetUniformLocation(shader.id, name.cstring), 1.GLsizei, GL_FALSE, value.caddr)
